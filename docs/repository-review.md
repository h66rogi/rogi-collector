...
이 문서는 **반입 전 조사 시점**의 근거와 초기 후보를 보존한 기록이다. 이후 기존 8개 모듈을 출발점으로 가져오라는 사용자 확정에 따라 보조 모듈도 선별 반입했다.
현재 파일 목록은 [반입 기록](source-imports.md), 실행 순서는 [구현 계획](implementation-plan.md), 완료 여부는 [구현 상태](implementation-status.md)를 따른다.

...
names='postgres-admin-password postgres-migrate-password migrate.pgpass redis-password discover.env coordinator.env worker.env query.env data-api.env archive-exporter.env tunnel-token public-api-db-password cookie-auth.env app-migrate.env tls-ca.pem tls-ca.key'
...
rm "$tmp/run/data-api.env" "$tmp/run/source-secrets.123/data-api.env" "$tmp/run/archive-exporter.env" "$tmp/run/source-secrets.123/archive-exporter.env" "$tmp/run/tunnel-token" "$tmp/run/source-secrets.123/tunnel-token" "$tmp/run/public-api-db-password" "$tmp/run/source-secrets.123/public-api-db-password"
...
make_valid;rm "$tmp/run/public-api-db-password";ln -s "$tmp/escape" "$tmp/run/public-api-db-password"
if "$root/tools/ops/validate-runtime-secrets.sh" "$tmp/run" 2>/dev/null;then echo 'invalid optional database password accepted' >&2;exit 1;fi

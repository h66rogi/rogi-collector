...
def main():
 ...
 disk=shutil.disk_usage('/srv/rogi-collector');assessment['dataDiskHealthy']=data_disk_healthy(disk);assessment['ok']=assessment['ok'] and assessment['dataDiskHealthy']
 status={'checkedAt':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),'sourceSha':manifest.get('sourceSha') if isinstance(manifest,dict) else None,'receipt':receipt,**assessment,'dataDisk':disk._asdict(),'latestBackup':{'path':str(latest),'ageSeconds':int(time.time()-latest.stat().st_mtime)} if latest else None,'capabilities':{'grpc7443':'mtls-collector-v1','soopCollector':'single-configured-channel; inspect channel status separately'}}

...
{"schemaVersion":1,"product":"rogi-collector","profile":"soop-single-channel","sourceSha":"1111111111111111111111111111111111111111","releaseId":"test-release","contractVersion":"v1","composeSha256":"$compose_sha","runtimeFiles":$runtime_json,"images":{"postgres":"registry.example/test/postgres@sha256:$digest","redis":"registry.example/test/redis@sha256:$digest","discover":"registry.example/test/discover@sha256:$digest","coordinator":"registry.example/test/coordinator@sha256:$digest","worker":"registry.example/test/worker@sha256:$digest","query":"registry.example/test/query@sha256:$digest","cloudflared":"registry.example/test/cloudflared@sha256:$digest","cookie-auth":"registry.example/test/cookie-auth@sha256:$digest"},"migrations":[{"path":"deploy/migrations/001_foundation.sql","sha256":"$migration_sha"}],"runtimeNonSecret":{"composeProjectName":"rogi-collector","dataRoot":"/srv/rogi-collector","secretsRoot":"/run/rogi-collector","releaseRoot":"/opt/rogi-collector/app/current","queryBindIp":"10.0.0.10","postgresDatabase":"collector","postgresAdminUser":"collector_admin","postgresMigrateUser":"collector_migrate","capabilities":{"grpc7443":"mtls-collector-v1"}}}
EOF
cat > "$tmp/bin/systemctl" <<'EOF'
#!/bin/sh
echo "systemctl $*" >> "$COMMAND_LOG"
[ "${TRANSIENT_UNITS:-}" = 1 ] && [ "$1" = is-active ] && case "$*" in
 *rogi-collector-role@discover.service*)
  count_file=${COMMAND_LOG}.discover-count

...
for name in postgres-admin-password postgres-migrate-password migrate.pgpass redis-password discover.env coordinator.env worker.env query.env data-api.env cookie-auth.env app-migrate.env tls-ca.pem tls-ca.key; do

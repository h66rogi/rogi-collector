...
for unit in rogi-collector-host-ready.service rogi-collector-migrate.service rogi-collector-role@.service rogi-collector-update.service rogi-collector-update.timer rogi-collector-backup.service rogi-collector-backup.timer rogi-collector.target rogi-collector-tls.service rogi-collector-tls.timer; do
...
if [ -z "$host_prefix" ] && ! cmp -s "$source_root/deploy/cookie-auth.apparmor" /etc/apparmor.d/rogi-collector-cookie-auth; then
  install -m 0644 "$source_root/deploy/cookie-auth.apparmor" /etc/apparmor.d/rogi-collector-cookie-auth
  apparmor_parser -r /etc/apparmor.d/rogi-collector-cookie-auth
fi
$systemctl_bin daemon-reload
if [ "$update_only" = false ];then $systemctl_bin enable rogi-collector.target rogi-collector-update.timer rogi-collector-backup.timer rogi-collector-tls.timer;fi
